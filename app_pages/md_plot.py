...
def render_md_plot_page(page: PageDefinition) -> None:
    ...
    render_page_header(page, "上传 GROMACS/MD 分析结果文件，自动解析 XVG 和 MMPBSA 文件并生成统一 SCI 风格图表。")

    with st.form("md_plot_form", clear_on_submit=False):
        uploaded_files = st.file_uploader(
            "上传 XVG、MMPBSA 或 ZIP 文件",
            type=["xvg", "csv", "dat", "zip"],
            accept_multiple_files=True,
            help="支持 rmsd.xvg、rmsf.xvg、gyrate.xvg、sasa.xvg、hbond.xvg、FINAL_RESULTS_MMPBSA.csv/dat 等。",
        )

        st.subheader("绘图参数")
        c1, c2, c3 = st.columns(3)
        with c1:
            project_name = st.text_input("项目名称", value="MD_project")
            time_unit = st.selectbox("时间单位", ["ps", "ns"], index=1)
            convert_ps_to_ns = st.checkbox("将 ps 转换为 ns", value=True)
        with c2:
            figure_width = st.number_input("图像宽度", min_value=2.0, max_value=20.0, value=6.5, step=0.5)
            figure_height = st.number_input("图像高度", min_value=2.0, max_value=20.0, value=4.8, step=0.5)
            dpi = st.number_input("DPI", min_value=72, max_value=1200, value=300, step=50)
        with c3:
            font_size = st.number_input("字体大小", min_value=6, max_value=24, value=11, step=1)
            make_summary = st.checkbox("生成组合图", value=True)
            export_pdf = st.checkbox("输出 PDF", value=True)
            export_clean_csv = st.checkbox("输出清洗 CSV", value=True)

        submitted = st.form_submit_button("运行 MD 绘图", type="primary")

    if submitted:
        if not uploaded_files:
            st.error("请至少上传一个 MD 结果文件。")
            return
        parameters = {
        ...
        try:
            with st.spinner("正在解析 MD 文件并生成图表..."):
...
def build_md_artifacts(
    ...
    artifacts = [
        ...
        {"label": "MD 汇总 CSV", "path": str(summary_path), "mime": "text/csv"},
        {"label": "MD 绘图日志", "path": str(log_path), "mime": "text/plain"},
    ]
    artifacts.extend({"label": f"清洗表格 {path.name}", "path": str(path), "mime": "text/csv"} for path in table_paths)
    for path in figure_paths:
        mime = "application/pdf" if path.suffix.lower() == ".pdf" else "image/png"
        artifacts.append({"label": f"MD 图表 {path.name}", "path": str(path), "mime": mime})
    artifacts.append({"label": "全部 MD 图表 ZIP", "path": str(zip_path), "mime": "application/zip"})
    return artifacts


def render_md_result(run_result: dict[str, Any]) -> None:
    st.subheader("自动识别到的 MD 文件类型")
